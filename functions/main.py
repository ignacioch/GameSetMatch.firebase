...
def addMatch(req: https_fn.Request) -> https_fn.Response:
    ...
    try:
        ...
        if "match_id" not in match_info.to_dict() and not getPlayerDetails(firestore_client,match_info.player_a_id):
            return https_fn.Response(f"Player A (ID= {match_info.player_a_id}) does not exist", status=400)
        if "match_id" not in match_info.to_dict() and not getPlayerDetails(firestore_client,match_info.player_b_id):
