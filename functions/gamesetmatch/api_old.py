...
def _addLeagueMatch(request_json) -> https_fn.Response:
    ...
    match_date = request_json.get("match_date")
    location = request_json.get("location")
    league_id = request_json.get("league_id") # Can be None
    group = request_json.get("group") # Can be None

    match_info = Match(player_a_id, player_b_id, score, match_date, location, match_id, league_id, group)
    logger.info(f"Adding Match. type=League match={match_info}")
    ...
    firestore_client: google.cloud.firestore.Client = firestore.client()
    ### first add the match to MATCHES
    new_match = addMatchToFirestore(firestore_client.transaction(), firestore_client, match_info)
    ### then update the league information
    league_info = addMatchToLeagueFirestore(firestore_client.transaction(), firestore_client, new_match, league_id)
    pass
...
def createLeague(req: https_fn.Request) -> https_fn.Response:
    ...
    ## Create a League
    league = League(league_name, area_id, 1, level, areas)
    logger.info(f"Incoming request={player}")

    # Attempts to create the league in Firestore and return its details
    try:
        league_info = createLeagueFirestore(league)
        return https_fn.Response(json.dumps({"league_info": league_info}), status=200, content_type="application/json")
    except ValueError as e:
        return https_fn.Response(str(e), status=400)
